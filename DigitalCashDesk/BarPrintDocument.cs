...
{
    internal class BarPrintDocument
    {
        ...
        internal BarPrintDocument(Order o, string TableName)
        {
            m_ToPrintOrder = o;
        ...
        void CashPrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            gObj.DrawString("Tisch: " + m_TableName, VerdanaFont, BrushBlack, x + 60, y);
            IncrementTextLine();
            gObj.DrawString("", VerdanaFont, BrushBlack, x, y);

            foreach (KeyValuePair<Product,int> p in m_ToPrintOrder.BoughtProducts)
            {
                if (Product.IDCAFE == p.Key.ProductTypeID ||
                    Product.IDCOCKTAIL == p.Key.ProductTypeID ||
                    Product.IDHEISS == p.Key.ProductTypeID ||
                    Product.IDKALT == p.Key.ProductTypeID)
                {
                    IncrementTextLine();
